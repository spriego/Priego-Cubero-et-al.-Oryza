...
#  (Figure 1A) ESPECIES TREE

```{r}
species_tree <- read.tree('input/species_tree')
```
...
plot_tree <- 
  ggtree(species_tree,
         branch.length='none') + 
  layout_rectangular() +
  geom_tiplab(size=4, linesize=.5) +
  #geom_text(aes(label = node)) + # To see which node do we want to flip
  xlim(NA, 25)

plot_tree <- flip(plot_tree, 18, 20)

```

Save plot
```{r}
ggsave('plots/species_tree.pdf',
      plot = plot_tree,
      dpi = 400,
      width = 10, 
      height = 5) 
...
CPS4: WLYV01017775.1.g81
KSL4: WLYV01017775.1.g77
MS: WLYV01017775.1.g80
CYP99A: WLYV01017775.1.g76
CYP76L11: WLYV01017775.1.g79

Ab initio annotation done in this manuscript for scaffold26

```{r}
O_coarctata <-
  read_table('input/Ocoarctata_scaffold26.gff3', 
           skip = 19, 
           col_names = c('Chr', 'Annotator', 'Feature', 'Start', 'End', 'c1', 'Orientation', 'c2', 'Metadata')) %>%
  filter(Feature == "gene") %>%
  filter(Chr == "WLYV01017775.1") %>%
  filter(Start %in% 1275743:1325473) %>%
  separate(Metadata, into = c("ID", "Gene"), sep = ";") %>%
  select(Chr, Start, End, Orientation, ID) %>%
  mutate(Gene = gsub("ID=", "", ID)) %>%
  select(!ID) %>%
  mutate(Genome = "O_coarctata") %>%
  type_convert() %>%
  mutate(Gene_Name = case_when(
    Gene == "WLYV01017775.1.g81" ~ "CPS4",
    Gene %in% c("WLYV01017775.1.g76") ~ "CYP99A",
    Gene %in% c("WLYV01017775.1.g79") ~ "CYP76L11",
    Gene %in% c("WLYV01017775.1.g80") ~ "MS",
    Gene == "WLYV01017775.1.g77" ~ "KSL4",
    TRUE ~ "Intervening" )
  ) %>% 
...
```{r}
all_genomes <- rbind(O_sativa, O_punctata, O_officinalis, O_eichingeri, O_rhizomatis, O_alta, O_australiensis, O_coarctata, O_brachyantha, O_granulata, L_perrieri)

# Create a factor variable with desired facet order
all_genomes$Genome <- factor(all_genomes$Genome, levels = c("O_sativa", "O_punctata", "O_officinalis", "O_rhizomatis", "O_eichingeri", "O_alta", "O_australiensis", "O_coarctata", "O_brachyantha", "O_granulata", "L_perrieri"))
...
geom_rect(xmin = 10000, xmax = 20000, ymin = -2, ymax = 2, fill = "white", color = "white",
  data = ~.x %>% 
    filter(Genome %in% c("O_rhizomatis", "O_eichingeri")
)) +
  geom_rect(xmin = 40000, xmax = 50000, ymin = -2, ymax = 2, fill = "white", color = "white",
  data = ~.x %>% 
    filter(Genome %in% c("O_eichingeri")
)) +
...
geom_rect(
  ...
  data = ~.x %>% 
    filter(Genome %in% c("O_australiensis", "O_granulata", "L_perrieri")
)) +
...
geom_rect(
  ...
  fill = "white", color = "white",
  data = ~.x %>% 
    filter(Genome %in% c("O_sativa", "O_punctata")
)) +
geom_rect(
  ...
  fill = "white", color = "white",
  data = ~.x %>% 
    filter(Genome %in% c("O_rhizomatis")
)) +
geom_rect(
...
geom_rect(
  ...
  fill = "white", color = "white",
  data = ~.x %>% 
    filter(Genome %in% c("O_coarctata")
)) +
geom_rect(
  ...
  fill = "white", color = "white",
  data = ~.x %>% 
    filter(Genome %in% c("O_brachyantha")
))
